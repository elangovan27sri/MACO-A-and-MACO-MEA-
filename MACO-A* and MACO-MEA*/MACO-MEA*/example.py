...
def main(nm,hovtrans,points,cities,rr,rc,rd,rx,ry,rz,pati):
...
#for battery percentage cal
    ...
    plot(pathi,rr,rc,rd,rx,ry,rz,xg,la,lb,lc,points,hovtrans,pati,lh)
import operator
import matplotlib.pyplot as plt
import numpy as np
import csv
...
def plot(pathi,rr,rc,rd,rx,ry,rz,xg,la,lb,lc,points,hovtrans,pati,lh):
    x = []
    y = []
    z = []
    gs=len(pathi)+1
    ...
    for i, txt in enumerate(indexx):
        if i>=1:
           ax.text(cx[i],cy[i],cz[i],'  {}'.format(txt),color='r',size=15)
    ax.text(x[0], y[0], 0, '   H',color='r',size=15)
    ax.scatter(x[0], y[0], 0,alpha=1, c='g', marker='s',s=150)
    ax.text(cx[0],cy[0],cz[0],'  0',color='r',size=15)
